									...
									<li>
										<div class="recent-div5">
											<div class="recent-div6">
												...
												<%-- -------------------쿠키 굽기---------------------- --%>
												<%-- 원본 <a target="_blank" rel="" class="recent-a" href="${pageContext.request.contextPath}/main/rmdt.do"> --%>
			
												<%-- 파라미터 GET으로 보내기 작업 --%>
												<a target="_blank" rel="" class="recent-a" href="${pageContext.request.contextPath}/main/rmdtsave.do?roomno=${item.roomno}">
			
												<%-- ---- 파라미터 FORM으로 보내기----- --%>
												<%-- <form method="post" action="${pageContext.request.contextPath}/main/rmdtsave.do?roomno=${item.roomno}"> --%>
													<%-- <input type="hidden" name="${item.roomno}" id="roomno" /> --%>
												<!-- </form> -->
...
<script type="text/javascript">
	...
	<script type="text/javascript">
		...
		var price1_from = price1_value.indexOf(${param.depositFrom});
		var price1_to = price1_value.indexOf(${param.depositTo});
		...
		var price2_from = price2_value.indexOf(${param.monthFrom});
		var price2_to = price2_value.indexOf(${param.monthTo});
		...
		var price3_from = price3_value.indexOf(${param.buyingFrom});
		var price3_to = price3_value.indexOf(${param.buyingTo});
		...
		var price4_from = price4_value.indexOf(${param.feeFrom});
		var price4_to = price4_value.indexOf(${param.feeTo});
		...
		$(function() {
			...
			$("#filter-reset3").click(function(e) {
				...
				/* slide5_value.reset();
				$("#filter5-value").html("무제한"); */
				var sizehref = '${pageContext.request.contextPath}/main/search.do?depositFrom=${param.depositFrom}&depositTo=${param.depositTo}&monthFrom=${param.monthFrom}&monthTo=${param.monthTo}&buyingFrom=${param.buyingFrom}&buyingTo=${param.buyingTo}&feeFrom=${param.feeFrom}&feeTo=${param.feeTo}&sizeFrom=0&sizeTo=99999';
				location.replace(sizehref);
			})
			...
			$("#filters-reset").click(function(e) {
				...
				slide5_value.reset();
				$(".inf").html("무제한"); */
				var resetUrl = "${pageContext.request.contextPath}/main/search.do?depositFrom=0&depositTo=999999&monthFrom=0&monthTo=999999&buyingFrom=0&buyingTo=999999&feeFrom=0&feeTo=9999&sizeFrom=0&sizeTo=99999";
